...
function getBaselineIndex(kind){
  if(kind === "acc") return firstValidIndex(accuniq, ["peso","smm","bfm","pbf","vfl","vfa"]);
...
function formatDateButton(value){
  return formatDateFriendly(value, {compact:true, shortMonth:true, monthOnlyFull:false});
}

function formatDateRange(label, dates, baseIndex, activeIndex){
  const baseDate = formatDateFriendly(dates?.[baseIndex]);
  const activeDate = formatDateFriendly(dates?.[activeIndex]);
...
function renderDateButtons(){
  ...
  const createGroup = (title, dates, activeIndex, onClick) => {
    ...
    dates.forEach((d,i)=>{
      ...
      btn.textContent = formatDateButton(d);
      btn.title = formatDateFriendly(d);
...
function drawLineChart(svgId, dates, metric, activeIndex){
  ...
  vals.forEach((v,i)=>{
    ...
    c.addEventListener("mousemove",ev=>showTip(ev,`${formatDateFriendly(dates[i])}<br><b>${metric.label}</b>: ${fmt(v)} ${metric.unit}`));
  ...
  dates.forEach((dt,i)=>{
    ...
    t.textContent = formatDateButton(dt).split("\n")[0];
  });
}
...
function detailCompareList(obj, baseIndex, activeIndex, keys, dates, label){
  const baseDate = formatDateFriendly(dates?.[baseIndex]);
  const activeDate = formatDateFriendly(dates?.[activeIndex]);
  let rows = "";
  keys.forEach(k=>{
  ...
  return `<div class="detailMeta">Punto inicial: <b>${baseDate}</b> · Fecha activa: <b>${activeDate}</b></div>
    <table class="detailTable"><tr><th>Métrica</th><th>Inicio</th><th>Activo</th><th>Cambio</th></tr>${rows}</table>`;
}
function renderDetails(){
  const compDate = formatDateFriendly(compDates[state.compIndex]);
  const bioDate = formatDateFriendly(accDates[state.accIndex]);
  const compBase = getBaselineIndex("comp");
  const accBase = getBaselineIndex("acc");
  const badgeText = state.view === "composition"
    ? `${labels.composition || "Plicometría"}: inicio ${formatDateFriendly(compDates[compBase])} → activo ${compDate}`
    : state.view === "accuniq"
      ? `${labels.bioimpedance || "Bioimpedancia"}: inicio ${formatDateFriendly(accDates[accBase])} → activo ${bioDate}`
      : `${labels.composition || "Plicometría"}: ${formatDateFriendly(compDates[compBase])} → ${compDate} · ${labels.bioimpedance || "Bioimpedancia"}: ${formatDateFriendly(accDates[accBase])} → ${bioDate}`;
  document.getElementById("activeDateBadge").textContent = badgeText;
  document.getElementById("compositionDetail").innerHTML = detailCompareList(composition,compBase,state.compIndex,["peso","grasaPct","grasaKg","magra","pliegues","cintura","abdomen"],compDates,labels.composition);
  document.getElementById("accuniqDetail").innerHTML = detailCompareList(accuniq,accBase,state.accIndex,["peso","smm","bfm","pbf","vfl","vfa"],accDates,labels.bioimpedance);
}
...
function renderTables(){
  ...
  document.getElementById("accuniqTable").innerHTML = makeTable(accuniq,accBase,state.accIndex,["peso","smm","bfm","pbf","vfl","vfa"]);
  const compRange = formatDateRange(labels.composition || "Plicometría", compDates, compBase, state.compIndex);
  const bioRange = formatDateRange(labels.bioimpedance || "Bioimpedancia", accDates, accBase, state.accIndex);
...
function generateExecutiveInsights(){
  ...
  const perimeterParts = [insightSentence(cintura), insightSentence(abdomen)].filter(Boolean);
  if(perimeterParts.length) items.push(`<b>Perímetros clave:</b> ${perimeterParts.join(" ")}`);
...
function buildAiPayload(){
  ...
  return {
    ...
    comparison: {
      ...
      bioimpedance: {initialDate: accDates[accBase] || null, activeDate: accDates[state.accIndex] || null, metrics: pickSummaries(accuniq,accBase,state.accIndex,["peso","smm","bfm","pbf","vfl","vfa"])}
    }
  };
}
...
async function requestAiSummary(){
  ...
  try{
    ...
    dashboardInsights.push(`<b>AI pendiente:</b> ${error.message}. Se mostró lectura automática local como respaldo.`);
    renderInsights();
  } finally{
    btn.disabled = false;
