...
function renderMetricButtons(){
  ...
  const metricKeys = state.view==="accuniq"
    ...
    : state.view==="composition"
      ...
      : ["peso","grasaPct","grasaKg","magra","pliegues","brazo","cintura","abdomen","cadera","muslo","smm","bfm","pbf","vfl","vfa"];
  box.innerHTML="";
...
function renderNormChart(){
  ...
  const series = state.view==="accuniq"
    ? [["Peso",accuniq.peso],["SMM",accuniq.smm],["BFM",accuniq.bfm],["PBF",accuniq.pbf],["VFL",accuniq.vfl],["VFA",accuniq.vfa]]
  ...
  const w = svg.clientWidth || 600, h = svg.clientHeight || 280;
  const pad={l:58,r:105,t:28,b:42};
  ...
  prepared.forEach(({name,metric:m,vals})=>{
    ...
    if(active!==null && active!==undefined){
      el("text",{x:w-pad.r+8,y:y(active)+4,fill:m.color,class:"valueText"}).textContent=`${name} ${active>0?"+":""}${fmt(active)}%`;
    }
  });
  dates.forEach((dt,i)=>{
    if(i%2!==0 && dates.length>7 && i!==dates.length-1) return;
...
function renderDetails(){
  ...
  document.getElementById("accuniqDetail").innerHTML = detailCompareList(accuniq,accBase,state.accIndex,["peso","smm","bfm","pbf","vfl","vfa"],accDates,labels.bioimpedance,"acc");
}
...
function syncChartControls(){
  const metricChart = document.getElementById("metricBtnsChart");
  const dateChart   = document.getElementById("dateBtnsChart");
  const metricSrc   = document.getElementById("metricBtns");
  const dateSrc     = document.getElementById("dateBtns");
  if(metricChart && metricSrc) metricChart.innerHTML = metricSrc.innerHTML;
  if(dateChart   && dateSrc)   dateChart.innerHTML   = dateSrc.innerHTML;
  if(metricChart) metricChart.querySelectorAll("button[data-metric]").forEach(btn => {
    btn.onclick = () => { state.metric = btn.dataset.metric; renderAll(); };
  });
  if(dateChart) dateChart.querySelectorAll("button[data-comp-index]").forEach(btn => {
    btn.onclick = () => { state.compIndex = Number(btn.dataset.compIndex); renderAll(); };
  });
  if(dateChart) dateChart.querySelectorAll("button[data-acc-index]").forEach(btn => {
    btn.onclick = () => { state.accIndex = Number(btn.dataset.accIndex); renderAll(); };
  });
}
