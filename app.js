...
function renderMetricButtons(){
  ...
  const allVisible = [
    ...(showComp ? compKeys.filter(k => metricHasData(composition[k])) : []),
    ...(showAcc  ? accKeys.filter(k => metricHasData(accuniq[k])) : [])
  ];
  if(!allVisible.includes(state.metric) || !metricHasData(composition[state.metric] || accuniq[state.metric])){
    state.metric = allVisible[0] || "peso";
  }
}
function renderKpis(){
