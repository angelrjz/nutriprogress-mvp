...
let state = {view:"overview", metric:"pliegues", metricSource:"comp", compIndex:0, accIndex:0};

const DEFAULT_MONTH_GOALS = [
  {label:"Peso corporal", targetLabel:"71.8–72.3 kg"},
  {label:"% grasa corporal", targetLabel:"18.0–18.3%"},
  {label:"Masa grasa", targetLabel:"13.0–13.3 kg"},
  {label:"Masa magra", targetLabel:"≥59.0 kg"},
  {label:"Suma de pliegues", targetLabel:"94–96 mm"},
...
const COMPOSITION_METRICS = {
  peso: ["Peso corporal", "kg", true],
  grasaPct: ["% grasa corporal", "%", true],
  grasaKg: ["Masa grasa", "kg", true],
  magra: ["Masa magra", "kg", false],
  pliegues: ["Suma de pliegues", "mm", true],
...
const BIOIMPEDANCE_METRICS = {
  peso:  ["Peso corporal", "kg", true],
  smm:   ["Masa muscular esquelética (SMM)", "kg", false],
  bfm:   ["Masa grasa (BFM)", "kg", true],
  pbf:   ["% grasa corporal (PBF)", "%", true],
  vfl:   ["Nivel de grasa visceral", "nivel", true],
  bmi:   ["IMC", "kg/m²", true]
};
...
const HEADER_ALIASES = {
  ...
  smm: "smm",
  mmesqueletica: "smm",
  masamuscularesqueletica: "smm",
  skeletalmusclemass: "smm",
  skeletalmasamuscular: "smm",
  pbf: "pbf",
  percentbodyfat: "pbf",
  porcentajedegrasacorporal: "pbf",
  porcgrasa: "pbf",
  vfl: "vfl",
  grasavisceral: "vfl",
  visceral: "vfl",
  // VFA / área visceral NO es IMC. Se mantiene separado para no pisar el IMC.
  vfa: "vfa",
  areavisceral: "vfa",
  areadegrasavisceral: "vfa",
  visceralarea: "vfa",
  visceraldatarea: "vfa",
  bmi: "bmi",
  bmikgm: "bmi",
  ...
  imckgm2: "bmi",
  indicemasa: "bmi",
  indicedemasacorporal: "bmi",
  bodymassindex: "bmi",
  bodymassindexkgm: "bmi",
...
function metricHasData(m){
  return !!(m && Array.isArray(m.values) && m.values.some(v =>
...
function transformSheetRows(rows){
  ...
  const accRows = rows.filter(r => {
    const tipo = (r.tipo || "").toLowerCase().replace(/[áàä]/g,"a").replace(/[éèë]/g,"e").replace(/[íìï]/g,"i").replace(/[óòö]/g,"o").replace(/[úùü]/g,"u");
    // Acepta cualquier variante de bioimpedancia o equipo conocido
    if(tipo.includes("bio") || tipo.includes("acc") || tipo.includes("inbody") ||
       tipo.includes("tanita") || tipo.includes("seca") || tipo.includes("imc") ||
       tipo.includes("impedancia") || tipo.includes("composicion")) return true;
    // Fallback: fila que NO es plicometría y tiene al menos smm, bfm o pbf
    if(tipo.includes("plic")) return false;
    const hasBioMarker = r["smm"] || r["bfm"] || r["pbf"] || r["bmi"] || r["imc"] || r["vfl"];
    return !!hasBioMarker;
  });
  ...
  return {
    ...
    composition: {
      peso: metric("Peso corporal", "kg", pick(compRows,"peso"), colors.peso, true),
      grasaPct: metric("% grasa corporal", "%", pick(compRows,"grasaPct"), colors.grasaPct, true),
      grasaKg: metric("Masa grasa", "kg", pick(compRows,"grasaKg"), colors.grasaKg, true),
      magra: metric("Masa magra", "kg", pick(compRows,"magra"), colors.magra, false),
      pliegues: metric("Suma de pliegues", "mm", pick(compRows,"pliegues"), colors.pliegues, true),
    ...
    accuniq: (function(){
      ...
      const bmiVals = (()=>{
        const v = pick(accRows,"bmi");
        if(hasData(v)) return v;
        const v2 = pick(accRows,"imc");
        if(hasData(v2)) return v2;
        return Array.from({length: accRows.length}, () => null);
      })();
      return {
        peso: metric("Peso corporal", "kg", pick(accRows,"peso"), colors.peso, true),
        smm:  metric("Masa muscular esquelética (SMM)", "kg", pick(accRows,"smm"),  colors.smm,  false),
        bfm:  metric("Masa grasa (BFM)", "kg", pick(accRows,"bfm"),  colors.bfm,  true),
        pbf:  metric("% grasa corporal (PBF)", "%", pick(accRows,"pbf"),  colors.pbf,  true),
        vfl:  metric("Nivel de grasa visceral", "nivel", pick(accRows,"vfl"),  colors.vfl,  true),
...
function getMode(){
  ...
  if(state.metricSource === "acc" && metricHasData(accuniq[state.metric])) return "acc";
  if(state.metricSource === "comp" && metricHasData(composition[state.metric])) return "comp";
  return metricHasData(accuniq[state.metric]) && !metricHasData(composition[state.metric]) ? "acc" : "comp";
}
...
function renderDateButtons(){
  ...
  createGroup(labels.composition || "Plicometría", compDates, state.compIndex, (i)=>{
    ...
    state.metricSource = "comp";
    if(!metricHasData(composition[state.metric])) state.metric = metricHasData(composition.pliegues) ? "pliegues" : Object.keys(composition).find(k => metricHasData(composition[k]));
    renderAll();
  }, "comp");
  ...
  createGroup(labels.bioimpedance || "Bioimpedancia", accDates, state.accIndex, (i)=>{
    ...
    state.metricSource = "acc";
    if(!metricHasData(accuniq[state.metric])) state.metric = metricHasData(accuniq.pbf) ? "pbf" : Object.keys(accuniq).find(k => metricHasData(accuniq[k]));
    renderAll();
  }, "acc");
}
function renderTabs(){
...
function renderMetricButtons(){
  ...
  function makeGroup(title, keys, obj, source){
    const available = keys.filter(k => metricHasData(obj[k]));
    ...
    available.forEach(k => {
      const src = obj[k];
      if(!metricHasData(src)) return;
      const btn = document.createElement("button");
      btn.textContent = src.label + (src.unit ? " " + src.unit : "");
      btn.className = (k === state.metric && source === state.metricSource) ? "active" : "";
      btn.onclick = () => { state.metric = k; state.metricSource = source; renderAll(); };
      btns.appendChild(btn);
    });
    group.appendChild(btns);
    box.appendChild(group);
    return available.map(key => ({key, source}));
  }

  const visibleMetrics = [];
  if(showComp) visibleMetrics.push(...makeGroup(labels.composition || "Plicometría", compKeys, composition, "comp"));
  if(showAcc)  visibleMetrics.push(...makeGroup(labels.bioimpedance || "Bioimpedancia", accKeys, accuniq, "acc"));

  // Fallback if metric no longer exists or if duplicated keys (e.g. peso) point to the wrong source
  const isValid = visibleMetrics.some(m => m.key === state.metric && m.source === state.metricSource);
  if(!isValid){
    const fallback = visibleMetrics[0] || {key:"peso", source:"comp"};
    state.metric = fallback.key;
    state.metricSource = fallback.source;
  }
}
function renderKpis(){
  ...
  const cards = [
    {name:"Peso corporal", val:composition.peso?.values?.[state.compIndex], prev:composition.peso?.values?.[compBase], unit:"kg", color:colors.peso, goodDown:true},
    {name:"% grasa corporal", val:composition.grasaPct?.values?.[state.compIndex], prev:composition.grasaPct?.values?.[compBase], unit:"%", color:colors.grasaPct, goodDown:true},
    {name:"Suma de pliegues", val:composition.pliegues?.values?.[state.compIndex], prev:composition.pliegues?.values?.[compBase], unit:"mm", color:colors.pliegues, goodDown:true},
    ...
    {name:"Masa muscular esquelética", val:accuniq.smm?.values?.[state.accIndex], prev:accuniq.smm?.values?.[accBase], unit:"kg", color:colors.smm, goodDown:false},
    {name:"% grasa corporal bio", val:accuniq.pbf?.values?.[state.accIndex], prev:accuniq.pbf?.values?.[accBase], unit:"%", color:colors.pbf, goodDown:true}
  ];
...
function renderMainChart(){
  const mode = getMode();
  let src = mode === "acc"
    ? {data:accuniq,dates:accDates,index:state.accIndex,kind:"acc"}
    : {data:composition,dates:compDates,index:state.compIndex,kind:"comp"};
  let metric = src.data[state.metric];
  if(!metricHasData(metric)){
    const fallbackKey = Object.keys(src.data).find(k => metricHasData(src.data[k]));
    metric = fallbackKey ? src.data[fallbackKey] : {label:"Sin datos", unit:"", values:[], color:"#21d6e9", goodDown:true};
    if(fallbackKey) state.metric = fallbackKey;
  }
...
function renderNormChart(){
  ...
  svg.innerHTML = "";
  const normMode = (state.view === "accuniq" || (state.view === "overview" && state.metricSource === "acc")) ? "acc" : "comp";
  const series = normMode === "acc"
    ? [["Peso",accuniq.peso],["SMM",accuniq.smm],["BFM",accuniq.bfm],["PBF",accuniq.pbf],["VFL",accuniq.vfl],["IMC",accuniq.bmi]]
    : [["Peso",composition.peso],["% grasa",composition.grasaPct],["Masa grasa",composition.grasaKg],["Masa magra",composition.magra],["Pliegues",composition.pliegues],["Brazo",composition.brazo],["Cintura",composition.cintura],["Abdomen",composition.abdomen],["Cadera",composition.cadera],["Muslo",composition.muslo]];
  const dates = normMode === "acc" ? accDates : compDates;
  const activeIndex = normMode === "acc" ? state.accIndex : state.compIndex;
  const kind = normMode === "acc" ? "acc" : "comp";
