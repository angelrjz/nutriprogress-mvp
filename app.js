...
async function bootstrap(){
  try{
    ...
    if(data.accuniq?.bmi) console.log("[NP] bmi values from Sheet:", JSON.stringify(data.accuniq.bmi.values));
    else console.warn("[NP] bmi key missing from data.accuniq. Keys:", Object.keys(data.accuniq||{}));
    ...
    if(accuniq?.bmi) console.log("[NP] bmi after buildMetricSet:", JSON.stringify(accuniq.bmi.values));
    composition = buildMetricSet(data.composition || {}, COMPOSITION_METRICS, compDates.length);
    accuniq = buildMetricSet(data.accuniq || data.bioimpedancia || {}, BIOIMPEDANCE_METRICS, accDates.length);
